...
class monaLisa(geneticProblem):
    ...
    def genImage(self, height, width):
        ...
        for _ in range(numPolygons):
            ...
            color_variation = tuple(random.randint(max(colr - 10, 0), min(colr + 10, 255)) for colr in base_color)
    ...
    def fitnessFunction(self, chromosomes):
        return np.mean(colour.difference.delta_e.delta_E_CIE1976(self.targetImage, chromosomes))
    ...
    def crossover(self, parent1, parent2):
        ...
        if strategy == "split":
            ...
            if splitType == "horizontal":
                ...
                rightPart = parent2['image'].crop((splitPos, 0, self.width, self.height))
                ...
                bottomLeft = parent1['image'].crop((0, splitPosY, splitPosX, self.height))
                bottomRight = parent2['image'].crop((splitPosX, splitPosY, self.width, self.height))
    ...
    def mutate(self, chromosome, mutationRate=0.9):
        ...
        if random.random() < mutationRate:
            ...
            mutatedChromosome['fitness'] = self.fitnessFunction(mutatedChromosome['array'])
...
def evolutionaryAlgorithm(question, generations, parentScheme):
    ...
    for i in range(generations):
        ...
        print("Fittest Chromosome in generation {} has a fitness of: {:.2f}".format(i, bestFit))
...
def main():
    ...
    fittest_chromosome = evolutionaryAlgorithm(question, numGenerations, tournamentSelect)
